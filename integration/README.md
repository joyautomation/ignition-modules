...
- **`harness/gateway.go`** and **`gateway-project/integration-api/`**: how tests see Ignition. `integration-api` is an Ignition
  ...
  queryTagHistory, the gateway's navigation model, and its loaded module descriptors). It is deliberately not a side door into the module: tests observe the gateway the way a
...
| `TheModuleIdentifiesItsVendor` | the gateway parsed the vendor out of module.xml (the build injects it; the Gradle plugin has no setting for it) |
