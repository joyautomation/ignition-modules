...
#
# The DIRECTORY is mounted, not the file. Bind-mounting dev/gateway.gwbk directly meant that replacing it —
# which is exactly what taking a new backup does — pulled the mount source out from under a running
# container: Docker recreated it as a root-owned directory and then refused every `compose cp` to
# that container with "mkdirat restore.gwbk: file exists". A directory mount survives the file inside it
# being replaced.
services:
  gateway:
    volumes:
...
# Nautilus themes for Perspective, generated by tools/generate-perspective-theme.py. Seeded so they survive
